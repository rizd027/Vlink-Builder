...
const PreviewSection = memo(({ theme, profile, links, socials, layoutType, previewDevice = 'mobile', setPreviewDevice, isEditorHidden, isMobileView }) => {
    ...
    return (
        <section className={`flex ${isMobileView || isEditorHidden ? 'flex-1 justify-center' : 'hidden xl:flex'} ${isMobileView ? 'h-screen overflow-hidden' : ''} ${!isMobileView && !isEditorHidden ? (previewDevice === 'mobile' ? 'w-[380px]' : (previewDevice === 'tablet' ? 'w-[620px]' : 'w-[750px]')) : ''} border-l border-white/5 flex-col items-center justify-center py-8 px-4 md:px-6 bg-transparent relative z-10 transition-all duration-500 ease-in-out`}>
            {/* Device Switcher */}
                    ...
                    >
                        ...
                        <div className={`relative z-10 flex flex-col items-center px-6 py-10 gap-5 min-h-full mx-auto ${previewDevice === 'desktop' ? 'max-w-4xl' : previewDevice === 'tablet' ? 'max-w-2xl' : 'w-full'}`}>
                            ...
                            <div className={`flex flex-col items-center w-full transition-all duration-300 ${profile.headerLayout === 'hero' ? 'gap-0 mb-3' : 'gap-3.5'}`}>
                                <div className={`relative ${profile.headerLayout === 'hero' ? 'rounded-b-none' : 'rounded-full'} ${theme.headerAnimation && theme.headerAnimation !== 'none' ? `animate-${theme.headerAnimation}` : ''} ${theme.headerAnimation === 'sweep' ? 'overflow-hidden' : ''}`}>
                                    ...
                                    {profile.avatar ? (
                                        <img
                                            ...
                                            className={`rounded-full border-2 border-white/10 shadow-md relative z-10 object-cover 
                                                ${profile.headerSize === 'large' ? 'w-24 h-24' : 'w-18 h-18'}
                                    ...
                                    <div className={`rounded-full border-2 border-white/10 shadow-md relative z-10 flex items-center justify-center bg-white/5 
                                            ${profile.headerSize === 'large' ? 'w-24 h-24' : 'w-18 h-18'}
                                            ${profile.headerLayout === 'hero' ? 'rounded-b-none border-b-0' : ''}
                                            ${profile.avatar ? 'hidden' : 'flex'}
                                        `}>
                                        <User size={profile.headerSize === 'large' ? 40 : 28} className="text-white/20" />
                                    </div>
                                </div>
                                ...
                                `}>
                                    {theme.titleStyle === 'text' ? (
                                        <h2
                                            className={`${profile.headerSize === 'large' ? 'text-xl' : 'text-base'} ${theme.titleAnimation && theme.titleAnimation !== 'none' ? (theme.titleAnimation === 'sweep' ? 'animate-sweep-text' : `animate-${theme.titleAnimation}`) : ''}`}
                                            style={{
                                                fontFamily: theme.titleFont || 'Inter',
                                                fontWeight: theme.titleWeight || 700,
                                                textTransform: theme.titleTransform || 'none',
                                                fontSize: theme.titleSize ? `${theme.titleSize}px` : undefined,
                                        ...
                                        <div className={`bg-white/20 rounded-lg p-2 flex items-center justify-center ${profile.headerSize === 'large' ? 'w-32 h-10' : 'w-24 h-8'}`}>
